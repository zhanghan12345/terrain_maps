# -*- coding: utf-8 -*-
"""
中国地形图 —— 仅显示中国区域（含藏南、台湾、阿克塞钦、九段线、省界）
...
height = data['z'].sel(y=slice(-5, 60), x=slice(70, 150))


def interp_(data):
    new_lat = np.arange(-5, 60.01, RESOLUTION)
    new_lon = np.arange(70, 150.01, RESOLUTION)
...
leftlon, rightlon, lowerlat, upperlat = (70, 150, 10, 60)
...
# 仅保留中国区域 (需要maskout模块)
# maskout.shp2clip(cs, ax, os.path.join(DATA_DIR, 'bou2_4p.shp'), region='中国')

# 坐标轴
ax.set_xticks(np.arange(leftlon, rightlon + 1, 10), crs=ccrs.PlateCarree())
ax.set_yticks(np.arange(lowerlat, upperlat + 1, 10), crs=ccrs.PlateCarree())
...
cb = fig.colorbar(
    ...
    pad=0.06, fraction=0.05, aspect=30,
    ticks=np.arange(0, 3001, 200), label='m'
)
...
output_path = os.path.join(OUTPUT_DIR, 'Fig_China_Topography.png')
fig.savefig(output_path, dpi=OUTPUT_DPI, bbox_inches='tight')
print(f"中国地形图已保存: {output_path}")
